...
THIS IS THE SECOND ATTEMPT. The first shipped on 2026-09-12 and was reverted the same
morning because it DID NOT BIND: 35 distinct addresses produced 2 refusals and 61 produced
0. It carried its own address helper that read `X-Forwarded-For` from the RIGHT, which on
this deployment is the Cloudflare edge and varies by serving location, so a sweep scattered
across keys instead of accumulating against one.

The helper is gone. `dashboard.client_address` does that job now, on a chain read from the
raw header rather than inferred, and verified in production by claiming a false address and
watching the real one get recorded. Keying is no longer this file's business.

WHAT TO CHECK AFTER DEPLOYING, stated in advance so it is falsifiable. Two workers times
MAX_DISTINCT_EMAILS is the real ceiling, so the first refusal should arrive around probe 21
of a distinct-address sweep from one connection, and EVERY refusal should carry the SAME
key hash. Refusals scattered across several key hashes mean the key is unstable again and
the guard is not binding, which is exactly what was missed last time.
"""
import hashlib
import threading
import time
