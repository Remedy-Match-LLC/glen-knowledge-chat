...
def test_the_client_cannot_choose_its_own_key():
    """The forged element is on the LEFT, Render's appended one on the RIGHT."""
    assert trusted_client_ip("203.0.113.77, 66.8.150.120", "") == "66.8.150.120"


def test_a_long_forged_chain_still_resolves_to_the_appended_hop():
    """An attacker padding the header must not push the trusted element out of reach."""
    forged = ", ".join(f"10.0.0.{i}" for i in range(50))
    assert trusted_client_ip(f"{forged}, 66.8.150.120", "") == "66.8.150.120"


def test_no_header_falls_back_to_remote_addr():
    assert trusted_client_ip("", "66.8.150.120") == "66.8.150.120"
    assert trusted_client_ip("", "") == "anon"


def test_the_first_element_is_never_returned_when_a_second_exists():
    """The specific bug in every existing auth route: `.split(",")[0]`."""
    xff = "203.0.113.77, 66.8.150.120"
    assert trusted_client_ip(xff, "") != xff.split(",")[0].strip()


def test_ipv6_collapses_to_a_64():
    """One IPv6 allocation is enormous; without this a sweep walks its own prefix."""
    assert trusted_client_ip("2001:db8::1", "").endswith("/64")


def test_unparseable_junk_is_keyed_verbatim_not_discarded():
    """Discarding it would let a client escape the limiter by sending garbage."""
    assert trusted_client_ip("not-an-ip", "") == "not-an-ip"


def test_more_proxies_reaches_further_right_not_further_left():
    assert trusted_client_ip("a, b, 1.2.3.4, 5.6.7.8", "", trusted_proxies=2) == "1.2.3.4"
