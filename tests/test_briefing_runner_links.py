...
def test_practice_better_is_gone_from_the_money_module():
    """Retired 2026-09-08. pb_data was called by both money summaries, and
    @cached re-raises with no stale value, so the dead credential 500'd
    /api/money/today and /api/money/week for weeks."""
    src = (_repo() / "dashboard" / "money.py").read_text()
    assert "def pb_data" not in src
    assert "def pb_token" not in src
...
def test_money_prompt_uses_qbo_ar_for_receivables():
    ...
    # Practice Better was retired on 2026-09-08. Naming a dead system in the
    # prompt invited the model to report figures that can no longer be fetched.
