...
def _fresh_limiter(monkeypatch):
    """A clean limiter per test, so tests cannot bleed into each other.

    Also disables the response-time pad. These routes now wait to a fixed 1.5s total so a
    known address and an unknown one cannot be told apart by the clock, which is correct
    and is tested in tests/test_response_timing.py. Left on, ONE test here takes 16
    seconds, because each test makes dozens of requests. Slow tests get deleted, and these
    are about the limiter rather than about the pad.
    """
    monkeypatch.setattr(appmod, "_AUTH_LIMITER", auth_limits.EmailProbeLimiter())
    monkeypatch.setattr(appmod, "_equalise_response_time", lambda *a, **k: None)
    yield
...
# The production chain, read from the raw header on 2026-09-12:
#     [ anything the client wrote ] , <the client's real address> , <Cloudflare edge>
# Tests must build that shape or they exercise a header that cannot occur. The restored
# tests sent a bare two-element chain, which under the corrected hop count means "the
# client wrote nothing", so a value meant to be forged was read as genuine.
CLOUDFLARE = "172.68.129.174"


def _chain(ip, forged=None):
    """The header as Cloudflare and Render would actually deliver it."""
    parts = ([forged] if forged else []) + [ip, CLOUDFLARE]
    return ", ".join(parts)


def _post(client, route, email, ip="9.9.9.9", forged=None):
    body = {"email": email}
    if "password-login" in route:
        body["password"] = "irrelevant-but-long-enough"
    return client.post(route, json=body,
                       headers={"X-Forwarded-For": _chain(ip, forged)})
...
def test_a_forged_forwarding_header_cannot_reset_the_limit(client, route):
    ...
    r = _post(client, route, "next@example.invalid", ip="7.7.7.7", forged="203.0.113.99")
...
def test_the_logged_client_key_is_the_trusted_hop_not_the_forged_one(client, capsys, monkeypatch):
    ...
    monkeypatch.setattr(appmod, "_AUTH_LIMITER", auth_limits.EmailProbeLimiter())
    for i in range(auth_limits.MAX_DISTINCT_EMAILS):
        _post(client, "/portal/login-request", f"filler{i}@example.invalid",
              ip="8.8.4.4", forged="203.0.113.99")
    capsys.readouterr()
    _post(client, "/portal/login-request", "next@example.invalid",
          ip="8.8.4.4", forged="203.0.113.99")
    out = capsys.readouterr().out
