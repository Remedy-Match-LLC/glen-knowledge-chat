...
# --- the profile page has to be reachable -------------------------------
#
# 2b shipped /practitioner/profile and emailed practitioners a link to it,
# but no workspace page linked to it: once the session cookie was set, a
# practitioner who navigated away could not get back. A surface only its
# own page links to is a surface nobody finds twice.

_WORKSPACE_PAGES = ["practitioner-portal.html", "practitioner-dropship.html",
                    "practitioner-settings.html", "practitioner-profile.html"]


@pytest.mark.parametrize("page", _WORKSPACE_PAGES)
def test_every_workspace_page_links_to_the_profile(page):
    ...
    assert '/practitioner/profile' in nav.group(0), \
        f"{page} nav gives no way to reach the public profile page"


def test_invite_lands_where_the_email_asked(monkeypatch):
    """A profile invite must return_to the profile, not the portal home."""
    sent = {}
    ...
    assert "return_to=%2Fpractitioner%2Fprofile" in sent["url"]
    assert "token=TOK" in sent["url"]


def test_invite_default_is_still_the_portal(monkeypatch):
    """Callers that pass no return_to keep the old landing surface."""
    sent = {}
    monkeypatch.setattr(appmod._pp, "create_magic_link_token",
                        lambda pid, email, ttl_min=None: "TOK")
    monkeypatch.setattr(appmod, "_send_practitioner_magic_link",
...
def test_invite_return_to_is_still_allowlisted(monkeypatch):
    """return_to reaches the same allowlist every other magic link uses."""
    sent = {}
    monkeypatch.setattr(appmod._pp, "create_magic_link_token",
                        lambda pid, email, ttl_min=None: "TOK")
    monkeypatch.setattr(appmod, "_send_practitioner_magic_link",
    ...
    assert "evil.example" not in sent["url"]
    assert "return_to=%2Fpractitioner%2Fportal" in sent["url"]


def test_confirm_page_wording_follows_the_destination(monkeypatch):
    """The sign-in confirm page said 'your order' to someone invited to write
    a page. The button has to name where it actually goes."""
    monkeypatch.setattr(appmod._pp, "validate_magic_link", lambda t: True)
    c = appmod.app.test_client()

    profile = c.get("/practitioner/login-verify"
                    "?token=T&return_to=/practitioner/profile").get_data(as_text=True)
    assert "my page" in profile
    assert "my order" not in profile
