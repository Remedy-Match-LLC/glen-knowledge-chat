...
# ── phone: validate (pure) + write (Task 4 fix round 1) ───────────────────
# Her number is DISPLAYED to a client, never dialled programmatically, so
# validate_phone only rejects input that plainly cannot be a phone number --
# it does not enforce a single international format. See the function's own
# docstring in dashboard/practitioner_portal.py for the reasoning.

def test_validate_phone_accepts_plain_digits_and_common_punctuation():
    from dashboard.practitioner_portal import validate_phone
    for raw in ["+1 555-0100", "(907) 555-0100", "907.555.0100", "15550100"]:
        clean, err = validate_phone(raw)
...
def test_validate_phone_rejects_too_few_digits():
    from dashboard.practitioner_portal import validate_phone
    clean, err = validate_phone("+1-2-3")
...
def test_validate_phone_rejects_letters():
    from dashboard.practitioner_portal import validate_phone
    clean, err = validate_phone("call me maybe")
    assert clean is None
    assert err
...
def test_validate_phone_rejects_too_long():
    from dashboard.practitioner_portal import validate_phone
    clean, err = validate_phone("1" * 40)
    assert clean is None
    assert err


class _PhoneWriteCur:
    """Records every execute() call so a test can assert the UPDATE is
    scoped to exactly the phone column and one id -- practitioners is the
    same table the profile publish path writes, and a broader UPDATE here
    could clobber a field another surface owns."""
    def __init__(self):
        self.calls = []
    def execute(self, sql, params=()):
...
def test_set_practitioner_phone_scopes_update_to_column_and_id(monkeypatch):
    import db_supabase
    from dashboard import practitioner_portal as pp
    cur = _PhoneWriteCur()
    monkeypatch.setattr(db_supabase, "supabase_cursor", lambda: _FakeCtx(cur))
    clean, err = pp.set_practitioner_phone(PID, "+1 555-0100")
    assert err is None
    assert clean == "+1 555-0100"
    assert len(cur.calls) == 1
    sql, params = cur.calls[0]
    assert sql.startswith("UPDATE practitioners SET phone=")
    assert "WHERE id=" in sql
    assert params == ("+1 555-0100", PID)


def test_set_practitioner_phone_rejects_bad_input_without_writing(monkeypatch):
    """A validation failure must never reach the UPDATE at all."""
    import db_supabase
    from dashboard import practitioner_portal as pp
    cur = _PhoneWriteCur()
    monkeypatch.setattr(db_supabase, "supabase_cursor", lambda: _FakeCtx(cur))
    clean, err = pp.set_practitioner_phone(PID, "nope")
    assert clean is None
    assert err
    assert cur.calls == [], "a rejected phone number must never reach the UPDATE"
